...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Package Splits ────────────────────────────────────────────────────────
    // Same structured shape + corrected unit math as actual_splits above — matches
    // PackagingRunDetailPage.jsx's completion insert now that it writes bare package_type +
    // size_spec instead of a flattened combined string (package_splits has no size_oz/
    // size_bbl columns, so volume_per_unit/total_volume carry the numeric size instead).
    const { error: psErr } = await svc.from('package_splits').insert([
      { batch_package_id: ids.bpkgHazy, brewery_id: b, package_type: 'Can',              size_spec: '16oz',                 units_packaged: 1820, volume_per_unit: 16,    total_volume: 7.3387, destination: 'distribution' },
      { batch_package_id: ids.bpkgHazy, brewery_id: b, package_type: 'Keg Sixth Barrel',  size_spec: '1/6 bbl (5.16 gal)',   units_packaged: 24,   volume_per_unit: 0.167, total_volume: 4.008,  destination: 'distribution' },
      { batch_package_id: ids.bpkgHazy, brewery_id: b, package_type: 'Taproom Draft',     size_spec: '16oz pour',           units_packaged: 185,  volume_per_unit: 16,    total_volume: 0.746,  destination: 'taproom' },
    ])
    check(psErr, 'insert package_splits')

    // ── Distribution Accounts (4) ─────────────────────────────────────────────
    const { error: daErr } = await svc.from('distribution_accounts').insert([
      { id: ids.distRetailer1, brewery_id: b, account_name: 'The Taproom District',        account_type: 'bar',      contact_name: 'Marcus Webb',  contact_email: 'marcus@taproomdistrict.example', contact_phone: '555-0201', payment_terms: 'net_30', is_active: true, address: '1240 N Michigan Ave, Chicago, IL 60611', notes: 'Best on-premise account — reorders every 3 weeks',           preferred_package_types: ['Keg Sixth Barrel'], contacts: [{ name: 'Marcus Webb',  email: 'marcus@taproomdistrict.example', phone: '555-0201', role: 'Buyer',   is_primary: true }], pricing: [{ package_type: 'Keg Sixth Barrel', price_per_unit: 145.00 }] },
      { id: ids.distRetailer2, brewery_id: b, account_name: 'Independent Bottle Shop',     account_type: 'retailer', contact_name: 'Sarah Kim',    contact_email: 'sarah@indbottle.example',         contact_phone: '555-0202', payment_terms: 'net_15', is_active: true, address: '842 W Belmont Ave, Chicago, IL 60657',   notes: 'Craft-focused — prefers local and seasonal releases',        preferred_package_types: ['Can'],              contacts: [{ name: 'Sarah Kim',    email: 'sarah@indbottle.example',        phone: '555-0202', role: 'Owner',   is_primary: true }], pricing: [{ package_type: 'Can',               price_per_unit: 1.85   }] },
      { id: ids.distBar1,      brewery_id: b, account_name: 'Craft & Draft Bar',           account_type: 'bar',      contact_name: 'Dave Torres',  contact_email: 'dave@craftdraft.example',          contact_phone: '555-0203', payment_terms: 'cod',    is_active: true, address: '560 W Hubbard St, Chicago, IL 60654',    notes: 'Tap takeover opportunity in Q3 — follow up on keg returns', preferred_package_types: ['Keg Sixth Barrel'], contacts: [{ name: 'Dave Torres',  email: 'dave@craftdraft.example',        phone: '555-0203', role: 'Manager', is_primary: true }], pricing: [{ package_type: 'Keg Sixth Barrel', price_per_unit: 140.00 }] },
      { id: ids.distTaproom,   brewery_id: b, account_name: 'Adaptive Taproom (in-house)', account_type: 'taproom',  is_active: true,                                                                                                                                                                                                                                                                                                                                                                                       pricing: [{ package_type: 'Taproom Draft',    price_per_unit: 7.50   }] },
    ])
