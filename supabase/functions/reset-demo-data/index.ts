...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: ingErr } = await svc.from('ingredients').insert([
      { id: ids.ingPale,       brewery_id: b, name: 'Pale Malt 2-Row',  category: 'Malt/Grain', unit: 'lb',     current_price_per_unit: 0.65, current_stock_quantity: 2000, stock_unit: 'lb',     reorder_threshold: 500, is_active: true, supplier_name: 'Great Western Malting' },
      { id: ids.ingMunich,     brewery_id: b, name: 'Munich Malt',       category: 'Malt/Grain', unit: 'lb',     current_price_per_unit: 0.85, current_stock_quantity: 500,  stock_unit: 'lb',     reorder_threshold: 200, is_active: true, supplier_name: 'Briess Malt' },
      { id: ids.ingCrystal,    brewery_id: b, name: 'Crystal 60L',       category: 'Malt/Grain', unit: 'lb',     current_price_per_unit: 0.95, current_stock_quantity: 200,  stock_unit: 'lb',     reorder_threshold: 100, is_active: true, supplier_name: 'Briess Malt' },
      { id: ids.ingRoasted,    brewery_id: b, name: 'Roasted Barley',    category: 'Malt/Grain', unit: 'lb',     current_price_per_unit: 1.10, current_stock_quantity: 150,  stock_unit: 'lb',     reorder_threshold: 75,  is_active: true, supplier_name: 'Briess Malt' },
      { id: ids.ingOats,       brewery_id: b, name: 'Flaked Oats',       category: 'Adjunct',    unit: 'lb',     current_price_per_unit: 0.70, current_stock_quantity: 300,  stock_unit: 'lb',     reorder_threshold: 100, is_active: true, supplier_name: 'Briess Malt' },
      { id: ids.ingCentennial, brewery_id: b, name: 'Centennial Hops',   category: 'Hops',       unit: 'oz',     current_price_per_unit: 2.10, current_stock_quantity: 32,   stock_unit: 'oz',     reorder_threshold: 16,  is_active: true, supplier_name: 'Yakima Chief' },
      { id: ids.ingCascade,    brewery_id: b, name: 'Cascade Hops',      category: 'Hops',       unit: 'oz',     current_price_per_unit: 1.85, current_stock_quantity: 25,   stock_unit: 'oz',     reorder_threshold: 30,  is_active: true, supplier_name: 'Yakima Chief' },
      { id: ids.ingCitra,      brewery_id: b, name: 'Citra Hops',        category: 'Hops',       unit: 'oz',     current_price_per_unit: 2.75, current_stock_quantity: 20,   stock_unit: 'oz',     reorder_threshold: 25,  is_active: true, supplier_name: 'Yakima Chief' },
      { id: ids.ingMosaic,     brewery_id: b, name: 'Mosaic Hops',       category: 'Hops',       unit: 'oz',     current_price_per_unit: 3.00, current_stock_quantity: 30,   stock_unit: 'oz',     reorder_threshold: 20,  is_active: true, supplier_name: 'Hop Union' },
      { id: ids.ingUs05,       brewery_id: b, name: 'US-05 Dry Yeast',   category: 'Yeast',      unit: 'packet', current_price_per_unit: 4.50, current_stock_quantity: 12,   stock_unit: 'packet', reorder_threshold: 4,   is_active: true, supplier_name: 'Fermentis' },
    ])
    check(ingErr, 'insert ingredients')

    // ── Recipes (4 + version 2 of Hazy IPA) ──────────────────────────────────
    const { error: recErr } = await svc.from('recipes').insert([
      { id: ids.recipeHazy,     brewery_id: b, name: 'Adaptive Hazy IPA', style: 'New England IPA',  base_batch_size: 15, base_batch_size_unit: 'barrels', target_og: 1.065, target_fg: 1.012, target_abv: 6.8, target_ibu: 45, target_margin_percentage: 65, version_number: 1, is_current_version: false },
      { id: ids.recipeHazyV2,   brewery_id: b, name: 'Adaptive Hazy IPA', style: 'New England IPA',  base_batch_size: 15, base_batch_size_unit: 'barrels', target_og: 1.065, target_fg: 1.012, target_abv: 6.8, target_ibu: 45, target_margin_percentage: 65, version_number: 2, is_current_version: true,  parent_recipe_id: ids.recipeHazy, version_notes: 'Increased Citra dry hop rate by 20% — responding to customer feedback on aroma. Added Mosaic dry hop.' },
      { id: ids.recipePivot,    brewery_id: b, name: 'Pivot Pale Ale',     style: 'American Pale Ale', base_batch_size: 15, base_batch_size_unit: 'barrels', target_og: 1.052, target_fg: 1.010, target_abv: 5.5, target_ibu: 35, target_margin_percentage: 60, version_number: 1, is_current_version: true },
      { id: ids.recipeStout,    brewery_id: b, name: 'Margin Stout',       style: 'American Stout',   base_batch_size: 10, base_batch_size_unit: 'barrels', target_og: 1.072, target_fg: 1.016, target_abv: 7.3, target_ibu: 40, target_margin_percentage: 62, version_number: 1, is_current_version: true },
      { id: ids.recipeBaseline, brewery_id: b, name: 'Baseline Lager',     style: 'American Lager',   base_batch_size: 20, base_batch_size_unit: 'barrels', target_og: 1.048, target_fg: 1.008, target_abv: 5.2, target_ibu: 18, target_margin_percentage: 55, version_number: 1, is_current_version: true },
    ])
    ...
    const { error: daErr } = await svc.from('distribution_accounts').insert([
      { id: ids.distRetailer1, brewery_id: b, account_name: 'The Taproom District',        account_type: 'bar',      contact_name: 'Marcus Webb',  contact_email: 'marcus@taproomdistrict.example', contact_phone: '555-0201', payment_terms: 'net_30', is_active: true },
      { id: ids.distRetailer2, brewery_id: b, account_name: 'Independent Bottle Shop',     account_type: 'retailer', contact_name: 'Sarah Kim',    contact_email: 'sarah@indbottle.example',         contact_phone: '555-0202', payment_terms: 'net_15', is_active: true },
      { id: ids.distBar1,      brewery_id: b, account_name: 'Craft & Draft Bar',           account_type: 'bar',      contact_name: 'Dave Torres',  contact_email: 'dave@craftdraft.example',          contact_phone: '555-0203', payment_terms: 'cod',    is_active: true },
    ...
    const { error: drErr } = await svc.from('distribution_records').insert([
      { brewery_id: b, batch_package_id: ids.bpkgHazy, account_id: ids.distRetailer1, account_name: 'The Taproom District',        account_type: 'bar',      package_type: '1/6 barrel keg',     quantity: 10,  delivery_date: '2026-03-30', sale_price_per_unit: 145.00, returnable_kegs: true,  keg_return_date: new Date(new Date('2026-03-30').getTime() + 20 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] },
      { brewery_id: b, batch_package_id: ids.bpkgHazy, account_id: ids.distRetailer2, account_name: 'Independent Bottle Shop',     account_type: 'retailer', package_type: '16oz Cans',          quantity: 240, delivery_date: '2026-03-30', sale_price_per_unit: 1.85 },
      { brewery_id: b, batch_package_id: ids.bpkgHazy, account_id: ids.distBar1,      account_name: 'Craft & Draft Bar',           account_type: 'bar',      package_type: '1/6 barrel keg',     quantity: 8,   delivery_date: '2026-04-01', sale_price_per_unit: 140.00, returnable_kegs: true,  keg_return_date: new Date(new Date('2026-04-01').getTime() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] },
      { brewery_id: b, batch_package_id: ids.bpkgHazy, account_id: ids.distTaproom,   account_name: 'Adaptive Taproom (in-house)', account_type: 'taproom',  package_type: 'Taproom Draft 16oz', quantity: 185, delivery_date: '2026-03-28', sale_price_per_unit: 7.50 },
      { brewery_id: b, batch_package_id: ids.bpkgHazy, account_id: ids.distRetailer1, account_name: 'The Taproom District',        account_type: 'bar',      package_type: '1/6 barrel keg',     quantity: 6,   delivery_date: '2026-04-05', sale_price_per_unit: 145.00, returnable_kegs: true,  keg_return_date: new Date(new Date('2026-04-05').getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] },
    ])
